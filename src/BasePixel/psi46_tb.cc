...
int CTestboard::AoutLevelChip(short position, short nTriggers, int trim[], int res[])
{
    ...
    for (unsigned i = 0; i < psi::ROCNUMROWS * psi::ROCNUMCOLS; i++) trimShort[i] = (short)trim[i];
    ...
    for (unsigned i = 0; i < psi::ROCNUMROWS * psi::ROCNUMCOLS; i++) res[i] = sdata[i];
    return 1;
}
...
int CTestboard::AoutLevelPartOfChip(short position, short nTriggers, int trim[], int res[], bool pxlFlags[])
{
    ...
    for (unsigned i = 0; i < psi::ROCNUMROWS * psi::ROCNUMCOLS; i++) pxlFlagsShort[i] = (pxlFlags[i] == true) ? 1 : 0;
...
void CTestboard::DoubleColumnADCData(int doubleColumn, short data[], unsigned readoutStop[])
{
...
int CTestboard::ChipThreshold(int start, int step, int thrLevel, int nTrig, int dacReg, int xtalk, int cals, int trim[], int res[])
{
    ...
    psi::Sleep(DEFAULT_DELAY);
    short sdata[psi::ROCNUMROWS * psi::ROCNUMCOLS] = {0};
    for (unsigned i = 0; i < psi::ROCNUMROWS * psi::ROCNUMCOLS; i++) sdata[i] = -1;
...
int CTestboard::SCurveColumn(int column, int nTrig, int dacReg, int thr[], int trims[], int chipId[], int res[])
{
    ...
    for (unsigned i = 0; i < 16 * psi::ROCNUMROWS; i++) threshold[i] = (short)thr[i];
    PUT_SHORTS(threshold, 16 * psi::ROCNUMROWS);
    for (unsigned i = 0; i < 16 * psi::ROCNUMROWS; i++) trim[i] = (short)trims[i];
    PUT_SHORTS(trim, 16 * psi::ROCNUMROWS);
    for (unsigned i = 0; i < 16; i++) chipIds[i] = (short)chipId[i];
